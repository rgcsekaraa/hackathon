"""
User profile context loader — pre-loads business details + availability
...
async def load_profile_context(
    ...
    # Check cache first
    cache_key = user_profile_id or "default"
    ...
    try:
        if user_profile_id:
            ...
            # Default: load the first profile (single-user MVP)
            result = await db.execute(
...
def get_profile_context_for_ai(context: dict) -> str:
    """
